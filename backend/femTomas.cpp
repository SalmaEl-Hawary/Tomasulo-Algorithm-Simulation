...
public:
    ...
    Instruction parseInstruction(const string &line, int address)
    {
        ...
        case BEQ:
        {
            string reg1, reg2;
            ss >> reg1 >> reg2;
            if (!reg1.empty() && reg1.back() == ',')
                reg1.pop_back();
            if (!reg2.empty() && reg2.back() == ',')
                reg2.pop_back();
            instr.rA = reg1[1] - '0';
            instr.rB = reg2[1] - '0';
    ...
    bool issueInstruction(Instruction &instr)
    {
        ...
        // Track speculation
        // Track speculation
        if (instr.type == BEQ || instr.type == CALL)
        { // ADD CALL HERE!
            ...
            if (instr.type == CALL)
            {
                robEntry.callOffset = instr.offset; // Already done, but keep
            }
        }
    ...
    int executeInstruction(ReservationStation &rs)
    {
        ...
        case ADD:
            result = rs.vj + rs.vk;
            if (debugMode)
                cout << "    ADD: " << rs.vj << " + " << rs.vk << " = " << result << "\n";
            break;
        case SUB:
            result = rs.vj - rs.vk;
            if (debugMode)
                cout << "    SUB: " << rs.vj << " - " << rs.vk << " = " << result << "\n";
            break;
        case NAND:
            result = ~(rs.vj & rs.vk);
            if (debugMode)
                cout << "    NAND: " << rs.vj << " NAND " << rs.vk << " = " << result << "\n";
            break;
        case MUL:
        {
    ...
    int getCyclesForType(InstructionType type)
    {
        ...
        {
        case LOAD:
            return 6; // 2 (address) + 4 (memory)
        case STORE:
            return 2; // 2 (address) + 4 (memory)
        case BEQ:
            return 1; // Compare + compute target
        case CALL:
            return 2; // Compute target + store return
        case RET:
            return 2; // Branch to R1
        case ADD:
            return 2;
        case SUB:
            return 2;
    ...
    string robStateToString(ROBState state)
    {
        ...
        case ISSUED:
            return "ISSUED";
        case EXECUTING_ROB:
            return "EXEC"; // <- Fixed: EXECUTING_ROB not EXECUTINGROB
        case WRITTEN:
            return "WRITTEN";
        case COMMITTED:
            return "COMMIT";
    ...
    void loadDefaultProgram()
    {
        ...
        instructionQueue.push_back(parseInstruction("LOAD R2, 4(R0)", 1));
        instructionQueue.push_back(parseInstruction("ADD R3, R1, R2", 2));
        instructionQueue.push_back(parseInstruction("SUB R4, R3, R1", 3));
        instructionQueue.push_back(parseInstruction("MUL R5, R1, R2", 4));
    ...
    void printResults()
    {
        ...
        double ipc = (instructionsCompleted > 0 && totalCycles > 0) ? (double)instructionsCompleted / totalCycles : 0.0;
        ...
        double mispredictionRate = (conditionalBranches > 0) ? (double)branchMispredictions / conditionalBranches * 100 : 0.0;
